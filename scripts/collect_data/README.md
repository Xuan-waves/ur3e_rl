...
- 姿态: 不再写入 `observation.state` 和 `action`；阻抗控制端保持固定末端姿态
- 当前不记录 `RL_mark/rlt_phase`；如需 RLT 标记，后续单独离线标注
...
- `observation.state`: `[tcp_x, tcp_y, tcp_z, gripper]`
- `action`: `[delta_tcp_x, delta_tcp_y, delta_tcp_z, target_gripper]`

默认情况下：

- `state[0:3]` 来自 `/ur3e_vr/robot_state` 的实际 TCP 位置
- `action[0:3]` 是 `/ur3e_vr/ik_target` 目标位置减去当前 `state[0:3]`
- `state[3]` 来自 robot state 的实际夹爪值；如果实际值缺失，会使用当前目标夹爪值兜底
- `action[3]` 优先来自 `/ur3e_vr/joint_target` 的夹爪目标，其次使用 VR command 或 robot state
...
`RL_mark/rlt_phase` 暂时不写入 LeRobot 数据。后续如果需要训练 RLT gate/阶段
分类器，建议先用离线脚本对已采集 episode 添加单独标签文件或独立 feature，
不要混入 `observation.state` 或 `action`。
...
里面包含同步窗口、基准相机、action 表达和同步统计。训练和 rollout 应以这个
report 为准，而不是只看当前 `config.py`。
...
- X: 开始记录当前 episode
- Y: 结束整个采集程序；如果当前 episode 正在记录或有未保存帧，会先丢弃当前 episode 再退出
- B: 第一次按下会停止当前 episode 采样并暂存到内存，同时打印本轮帧数；松开 B 后再次按下才确认保存
- Left upper trigger 完全按下: 停止记录并丢弃当前 episode，回到等待 X 的状态
- Left lower trigger / grip: 当前未使用
